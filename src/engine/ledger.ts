...
export interface Rules {
   ...
   * траты, их обгоняет только структура GreenLeaf. В режиме `ru` стоит
   * `activeGl` — см. правила режима в table.ts.
   */
  ...
  /**
   * Партия не кончается, пока последний играющий ещё в Круге: остальные уже
   * купили мечту, а он доигрывает до выхода. Цель Камиля 11.09: «на второй
   * круг выходят абсолютно все». Не задано — партия кончается, как раньше.
   */
  доигрыватьКруг?: boolean
  /** На сколько процентов базы дорожает мечта, когда на её клетку встаёт чужой. Не задано — 100. */
  dreamBumpPct?: number
  /** Потолок этой надбавки в процентах базы. Не задано — без потолка. */
  dreamBumpCapPct?: number
  /**
   * Обязательная трата второго круга — не больше стольких месяцев расходов игрока.
   * Не задано — сумма с клетки как есть.
   */
  ftLifeSpendMonths?: number
}
...
export const RIBA = {
   ...
   *
   * 🔴 БЫЛО ШЕСТЬ — И ЭТО НИ НА ЧТО НЕ ХВАТАЛО. Замер: полный кредит самой
   * бедной профессии (720 000) покрывал первый взнос ровно у ТРЁХ крупных
   * сделок из 54, то есть кнопка существовала, а смысла в ней не было.
   * Просьба Камиля: «надо сильно больше кредита».
   *
   * Шестнадцать выбрано не на глаз: столько нужно, чтобы средняя профессия
   * дотягивала до типичного первого взноса крупной сделки, а бедная — до
   * половины таких сделок. Замером ботов это не проверить — они кредит не
   * берут вовсе (за всю серию ноль обращений), так что цифра выведена из
   * взносов колоды, а не из партии.
   *
   * 🔴 Кредит остаётся ловушкой, и это осознанно: пока он открыт, неприятности
   * приходят чаще. Больше денег — больше соблазн, и урок от этого только
   * крепче.
   */
  limitIncomeMul: 16,
