...
строка(`вышли за ${ХОДОВ} шагов`, (с) => `${Math.round((с.вышли / с.мест) * 100)}%`)
строка('ходов стола к обрыву', (с) => `${медиана(с.ходов)}`)
/*
 * 🔴 ЭТО НЕ «СКОЛЬКО ВЫХОДИТ ВООБЩЕ». Партия обрывается на H шагах ботов, поэтому
 * «вышли» = «успели до обрыва», а длина у разных вариантов совпадает, потому что
 * все упираются в одну отсечку. 10.09 я показал Камилю «выходят 87% / 69%» и
 * «длина не меняется» — а до настоящего конца партии выходят 98%, и варианты
 * расходятся не по длине, а по тому, выходят ли ВСЕ за столом.
 */
console.log(`\n⚠️ партии обрываются на ${ХОДОВ} шагах: «вышли» — это «успели до обрыва», а не «сколько выходит вообще»`)
