...
import { glСтадия, glЛюдиИзНаборов } from './greenleaf'
...
// 3б. Наборы за пакет (решение Камиля 11.09): Платина 1 человек, Бриллиант 2, Корона 4.
for (const [пакет, наборов, людей] of [['platinum', 1, 1], ['diamond', 3, 2], ['crown', 6, 4]] as const) {
  const r = glЛюдиИзНаборов(пакет)
  if (r.наборов !== наборов || r.людей !== людей)
    провал(`наборы за пакет ${пакет}: ждём ${наборов} → ${людей}, вышло ${r.наборов} → ${r.людей}`)
}
...
// 5. Живой прогон: карточки с новыми полями реально доезжают до стола.
{
  ...
  const наборыВидели: { пакет: string; наборов: number; кто: string }[] = []
  for (let seed = 1; seed <= 40; seed++) {
    ...
    for (let ход = 0; ход < 600 && t.phase !== 'finished'; ход++) {
      ...
      for (const строка of t.log.slice(-6)) {
        ...
        const м = txt.match(/С пакетом «(Платина|Бриллиант|Корона)» пришло (\d+) (?:набор|набора|наборов) — (?:подключился|подключились) ([а-яё ]+)\./)
        if (м) наборыВидели.push({ пакет: м[1], наборов: Number(м[2]), кто: м[3] })
  ...
  /*
   * 🔴 Наборы — живьём: строка под карточкой обязана назвать ровно те числа,
   * что положены пакету вытянувшего. Иначе правило есть, а до стола не доехало.
   */
  const ЖДЁМ: Record<string, [number, string]> = { Платина: [1, 'один человек'], Бриллиант: [3, 'двое'], Корона: [6, 'четверо'] }
  if (!наборыВидели.length) провал('карточка «Наборы разошлись по родне» ни разу не сработала')
  for (const н of наборыВидели) {
    const [наборов, кто] = ЖДЁМ[н.пакет]
    if (н.наборов !== наборов || н.кто !== кто) провал(`наборы: ${н.пакет} дал «${н.наборов} → ${н.кто}», ждём «${наборов} → ${кто}»`)
  }
  console.log(`  наборы за пакет живьём: ${[...new Set(наборыВидели.map((н) => `${н.пакет} ${н.наборов} → ${н.кто}`))].join(' · ')}`)
  /*
