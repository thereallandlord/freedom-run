...
export interface TableSetup {
   ...
   *
   * 🔴 Часть НАСТРОЕК, а не правил: партия пересобирается из журнала ходов, и
   * колода при пересборке обязана получиться той же самой. Лежал бы выбор
   * где-то сбоку — журнал, записанный столом «без России», переигрался бы
   * полной колодой: другие карты, другие деньги, другой победитель.
   */
  рынки?: Рынок[]
}
...
export const THEME_RULES: Record<DeckTheme, Parameters<typeof setRules>[0]> = {
  ru: {
     ...
     *
     * 🔴 11.09 ПЕРЕСОБРАНО ПОД ЦЕЛЬ КАМИЛЯ: «на второй круг выходят абсолютно
     * все». 55% назначил я замером 30.08 — Камиль цифру не называл. И мерил я
     * тогда поверх ДВОЙНОГО роста трат: рядом работало старое правило «+3% от
     * дохода с активов каждую зарплату» (убрано, см. «Образ жизни» в зарплате).
     * Замер после чистки, 60 столов по 4, до выхода всех:
     *   55% от всего дохода: один без GreenLeaf — выходят все, ход 388; никто
     *     без GreenLeaf — 90%, 24 разорились; с GreenLeaf выход на 135-м ходу
     *   33% от работы и GreenLeaf: выходят ВСЕ за любым столом, разорений ноль;
     *     без GreenLeaf ход 277 (один) и 421 (никто), с GreenLeaf — 122–128
     * Траты растут вслед за зарплатой, делом без управляющего и структурой;
     * доход от активов — дело с управляющим, аренда, бумаги, франшиза — быт не
     * раздувает: эти деньги работают сами, их вкладывают дальше.
     */
    lifestyleCreepPct: 33,
    creepBase: 'activeGl',
    /*
     * Последний, кто ещё в Круге, доигрывает до выхода, даже если остальные уже
     * купили мечту. Без этого за столом, где трое взяли GreenLeaf, четвёртый
...
export function createTable(setup: TableSetup): Table {
   ...
   * будут указывать в колоду другой длины, и стол начнёт выдавать не те
...
function advance(t: Table, seatIdx: number, steps: number) {
  ...
  // Сколько прибавил рост трат за эти зарплаты — берём из кошелька, см. «Образ жизни» ниже.
  const доОбраза = t.seats[seatIdx].ledger.expenses.otherExpenses
  ...
  if (payouts > 0) {
     ...
     *
     * 🔴 СЧИТАЕМ ТОЛЬКО ПО КОШЕЛЬКУ. 19.08 здесь стояло своё правило: каждую
     * зарплату +3% от дохода с активов в расходы — навсегда и без потолка.
     * Через день ту же просьбу закрыл `подтянутьРасходы` (траты тянутся за
     * приростом дохода), а это правило не убрали, и два работали разом. Замер
     * 11.09: за долгую партию оно одно добавляло ~230 000 ₽/мес — больше
     * стартовых расходов профессии, — и игроки без GreenLeaf разорялись с
     * миллионами на руках: зарплата уже не покрывала раздутый быт. Здесь
     * теперь только уведомление о настоящем росте из кошелька — то, ради чего
     * Камиль его просил: «видно, как образ жизни съедает рост».
     */
    if (seat.track === 'rat') {
      const creep = l.expenses.otherExpenses - доОбраза
      if (creep >= 500) {
...
function resolveLanding(t: Table, seatIdx: number) {
  ...
  switch (space.type) {
    ...
    case 'lawsuit': {
      /*
       * 🔴 БЕДА ТЯНЕТСЯ ИЗ КОЛОДЫ, А НЕ ПРИБИТА К КЛЕТКЕ. Живая жалоба
       * Камиля: «сейчас выпадают одни иски». Так и было: на всё поле стояли
       * две клетки, и обе забирали половину наличных — разные подписи поверх
       * одной механики бедой не ощущаются.
       *
       * 🔴 КЛЕТКИ ОСТАЛИСЬ НА МЕСТАХ И С ПРЕЖНИМИ ТИПАМИ. Мечта игрока
       * хранится НОМЕРОМ клетки: тронешь доску — и все мечты за ней съедут на
       * чужие. Меняется только то, что на клетке происходит.
       *
       * 🔴 БЕДА СЧИТАЕТСЯ ОТ ДОХОДА, А НЕ ОТ КОШЕЛЬКА. «Половина налички»
       * наказывала ровно за то, что человек копил на мечту, и не трогала
       * того, кто всё вложил. У беды своя цена, и она не зависит от того,
       * сколько у тебя сегодня в кармане.
       */
      const колода = fastTroubles()
      const беда = колода.length
        ? колода[Math.min(колода.length - 1, Math.floor(rng(t, 6613) * колода.length))]
        : null
      const доход = fastTrackIncome(l, t.market.flow)

      if (беда && беда.вид === 'деньги') {
        const before = l.cash
        const цена = Math.max(50_000, Math.round((доход * (беда.месяцев ?? 3)) / 1000) * 1000)
        const реально = Math.min(l.cash, цена)
        seatLedgerEvent(t, seat.id, { type: 'ADJUST_CASH', amount: -реально })
        log(t, seat.id, `${беда.title}: ${money(реально)}`)
        плашка(t, seat.id, `${seat.name}: ${беда.title.toLowerCase()} — ${money(реально)}`, 'худо')
        t.pending = {
          kind: 'ftEvent',
          title: беда.title,
          text:
            беда.text +
            (реально < цена ? ` Счёт был на ${money(цена)}, но больше на счету не оказалось.` : ''),
          before,
      ...
      if (беда && беда.вид === 'просадка') {
        const пкт = беда.пкт ?? 25
        const месяцев = беда.месяцев ?? 4
        seatLedgerEvent(t, seat.id, { type: 'SET_FT_DIP', pct: пкт, paydays: месяцев })
        log(t, seat.id, `${беда.title}: доход просядет на ${пкт}% на ${месяцев} ${склонениеЗарплат(месяцев)}`)
        плашка(t, seat.id, `${seat.name}: ${беда.title.toLowerCase()} — доход −${пкт}% на ${месяцев} мес`, 'худо')
        t.pending = {
          kind: 'ftEvent',
          title: беда.title,
          text: `${беда.text} Доход просядет на ${пкт}% на ${месяцев} ${склонениеЗарплат(месяцев)} и вернётся сам.`,
          before: l.cash,
          after: l.cash,
        }
        t.phase = 'resolving'
      ...
      if (беда && беда.вид === 'пропуск') {
        const ходов = беда.ходов ?? 2
        t.seats[seatIdx] = { ...t.seats[seatIdx], skipTurns: ходов }
        log(t, seat.id, `${беда.title}: пропуск ${ходов} ходов`)
        плашка(t, seat.id, `${seat.name}: ${беда.title.toLowerCase()} — пропускает ${ходов} хода`, 'худо')
        t.pending = {
          kind: 'ftEvent',
          title: беда.title,
          text: `${беда.text} Деньги не тронуты — дела работают без вас, а вот ходов вы теряете ${ходов}.`,
          before: l.cash,
          after: l.cash,
          skip: ходов,
        }
      ...
      /*
       * Выбор «заплатить сразу или тянуть» — прежняя механика, но поводов
       * теперь несколько. Тянуть чуть дороже, чем заплатить: иначе выбор
       * фальшивый и платить никогда не имеет смысла.
       */
      const сумма = Math.max(50_000, Math.round((доход * 3) / 1000) * 1000)
      const месяцев = 6
      const просадкаПкт =
        доход > 0 ? Math.min(60, Math.max(5, Math.round(((сумма * 1.25) / месяцев / доход) * 100))) : 25
      t.pending = {
        kind: 'ftEvent',
        title: беда?.title ?? (space.type === 'taxAudit' ? 'Налоговая проверка' : 'Иск в суде'),
        text:
          беда?.text ??
...
function applyMarketAuto(t: Table, card: MarketCard): string[] {
  if (card.kind === 'bizEvent') {
    ...
    /*
     * 🔴 СОБЫТИЕ СЛУЧАЕТСЯ С ОДНИМ МЕСТОМ, А НЕ СО ВСЕМИ СРАЗУ (11.09). Раньше
     * карточка била по КАЖДОМУ подходящему активу: «арендатор съехал» обнулял
     * месяц дохода со всех квартир разом, а у кого сеть из шести точек —
     * «капремонт» закрывал все шесть. Хуже того, у тринадцати карточек не было
     * адресата вовсе, и «район застроили» резал навсегда доход и квартир, и
     * дел. В жизни съезжает жилец из одной квартиры и ломается вентиляция в
     * одной точке. Место выбираем случайно, но повторяемо — от номера хода и
     * карточки, не трогая общий счётчик случайности колод.
     */
    if (цели.length > 1) {
      const соль = [...card.id].reduce((h, ch) => (h * 31 + ch.charCodeAt(0)) >>> 0, 0)
      const номер = Math.floor(mulberry32(t.seed + t.turnCounter * 7919 + соль)() * цели.length)
      цели.splice(0, цели.length, цели[номер])
    }
...
function nextTurn(t: Table) {
  ...
  if (
    alive.length === 1 &&
    t.seats.filter((s) => !s.outOfGame).length > 1 &&
    t.winnerId &&
    /*
     * 🔴 Цель Камиля 11.09: «на второй круг выходят абсолютно все». При
     * включённом правиле последний, кто ещё в Круге, доигрывает до выхода,
     * а не выбывает вместе с концом партии.
     */
    !(RULES.доигрыватьКруг && alive[0].track === 'rat')
  ) {
...
function применитьСобытие(prev: Table, event: TableEvent): Table {
  ...
  switch (event.type) {
    ...
    case 'CHOOSE_DEAL': {
      ...
      /*
       * 🔴 ПОКА СВОЕГО ДЕЛА НЕТ — ДЕЛА ПОПАДАЮТСЯ ЧАЩЕ (просьба Камиля 05.09:
       * «бизнесы же должны покупать пораньше; сделать так, что если бизнес
       * уже есть, то процент выпадания дальше меньше»).
       *
       * 🔴 ЗАЧЕМ ЭТО ВООБЩЕ. Замер на 30 партиях: обычное дело заводили лишь
       * 38% игроков, и первое — на 115-м ходу из 175. Всё, что построено
       * вокруг дела (77 карточек событий, из них 21 с решением), почти никто
       * не видел: за партию такое событие выпадало примерно один раз. Дело —
       * главный урок игры (4,2% в месяц против 0,45% у квартиры), и до него
       * человек должен доходить не к концу партии.
       *
       * 🔴 ЭТО ЛОТЕРЕЯ, А НЕ ВЫДАЧА ИЗ КОЛОДЫ — как и перекос по бумагам
       * ниже. Тянуть «своё» через курсор общей колоды нельзя: при исчерпании
       * она перетасуется на длину горстки, и дальше из полусотни карт будут
       * доставаться первые несколько. На этом уже обжигались дважды.
       */
      const своиДела = seat.ledger.businesses.filter((b) => !b.gl).length
      const шансДела = своиДела === 0 ? 0.5 : своиДела === 1 ? 0.15 : 0.05
      let деловойПеркос = false
      if (card.kind !== 'business' && rng(t, 5503) < шансДела) {
        const дела = list.filter(
          (c) =>
            c.kind === 'business' &&
            !(c as { greenleaf?: boolean }).greenleaf &&
            // Второй раз то же самое заведение не предлагаем.
            !seat.ledger.businesses.some((b) => !b.gl && b.name === c.title),
        )
        if (дела.length) {
          const i = Math.min(дела.length - 1, Math.floor(rng(t, 5504) * дела.length))
          const кандидат = stockDrawPrice(t, scaled(дела[i]))
          if (dealDrawOk(t, кандидат, event.size)) {
            card = кандидат
            деловойПеркос = true
          }
        }
       ...
       *
       * 🔴 Не перебиваем перекос по делам: если дело только что подставили,
       * бумага его не вытесняет — иначе первый перекос был бы пустой работой.
       */
      const owned = new Set(
        t.seats.flatMap((s2) => s2.ledger.stocks.map((x) => x.symbol)),
      )
      if (!деловойПеркос && owned.size && card.kind !== 'stock' && rng(t, 5501) < 0.33) {
    ...
    case 'OFFER_CARD': {
      ...
      if (card.kind === 'stock') return prev
      /*
       * 🔴 ПАРТНЁРСКИЙ БИЗНЕС НЕ ПЕРЕПРОДАЁТСЯ И НЕ БЕРЁТСЯ В СКЛАДЧИНУ.
       *
       * Решение Камиля: «покупать может только тот, кому выпала карточка».
       * Одну дверь закрыли — вход соседа через условия; остались две: отдать
       * находку соседу и позвать его в долю. Обе кончались одинаково плохо:
       * сосед получал пакет БЕЗ СТРУКТУРЫ — мёртвую пустышку на 1 700 в
       * месяц, которая не растёт и ранга не даёт. Структура строится тем, кто
       * её строит, и передать её нельзя.
       */
      if ((card as { greenleaf?: boolean }).greenleaf) return prev
