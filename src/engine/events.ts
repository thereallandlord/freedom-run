...
export type TableEventBody =
  ...
  /**
   * Хозяин вручную поправляет счёт игрока после сбоя.
   *
   * 🔴 Живая просьба: «хосту нужен инструмент ручной компенсации после сбоя».
   * На игре 31 августа стол завис на карточке, часть действий пропала, и
   * вернуть человеку его деньги было нечем. Правка ВСЕГДА объявляется столу с
   ...
   */
  /** Желание на большом круге: взять или пройти мимо. */
  | { type: 'BUY_FT_WANT' }
  | { type: 'SKIP_FT_WANT' }
  ...
  /**
   * Вложиться в своё дело по карточке-предложению — или пройти мимо.
   * Деньги списываются только при `INVEST_BIZ`: пока решение не принято, на
   * руках у человека всё, что было, и обе кнопки считаются от одной суммы.
   */
  | { type: 'INVEST_BIZ' }
  | { type: 'SKIP_BIZ_INVEST' }
  /**
   * Продать свой актив, не дожидаясь карты рынка.
   *
   * 🔴 Решение Камиля: продавать можно ВСЕГДА, но рынок с характером — быстрая
   * продажа идёт со скидкой. Ждать покупателя по полной цене — это и есть
   * карта рынка, которая выпадает сама.
   */
  | { type: 'SELL_ASSET_NOW'; assetId: string }
