...
export function decideBotEvent(t: Table, rnd: () => number): TableEvent | null {
  ...
  switch (pending.kind) {
      ...
      if (card.kind === 'sellOffer') {
        ...
        for (const m of marketMatches(t, card.category)) {
          ...
          for (const a of m.assets) {
            ...
            /*
             * 🔴 ДЕНЬГИ СЕЙЧАС ПРОТИВ ДЕНЕГ ПОТОМ. Правило «продаю только выше
             * своего множителя» держало бота в объектах, которые почти не
             * кормят: замер 06.09 — 38 предложений о продаже за 30 партий,
             * принято 3. У двушки в Бутово поток 4 000 в месяц, и ждать
             * множителя 1,4 бессмысленно: выручка от продажи стоит больше, чем
             * этот объект принесёт за всю оставшуюся жизнь партии.
             *
             * Считаем горизонтом: если чистыми дают больше, чем актив принесёт
             * за 200 зарплат, — брать надо. Для нормального объекта это условие
             * недостижимо и ничего не меняет; срабатывает ровно там, где поток
             * ничтожен, а стоимость выросла.
             *
             * 🔴 Горизонт 200, а не 100. При сотне правило задевало нормальные
             * дешёвые активы, купленные налом: у них нет долга, поэтому выручка
             * велика по сравнению с маленьким потоком, — и бот распродавал
             * кладовку с окупаемостью 100 месяцев. Замер: выходы из Круга у
             * «нереального» просели с 60% до 51%.
             */
            const стоитДорожеЧемКормит = доходАктива > 0 && выручка >= доходАктива * 200
            const продаватьМожно = наПолосе
              ? ценаМечты > 0 &&
                l.cash + выручка >= ценаМечты &&
                послеПродажи >= totalExpenses(l) &&
                доМечты <= 8
              : price >= база * mult || стоитДорожеЧемКормит
            if (продаватьМожно) {
    ...
    case 'payday':
    // Закрытие ранга — новость, решать нечего.
    case 'glRank':
