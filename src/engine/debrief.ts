...
export function buildDebrief(table: Table, events: TableEvent[], seat: Seat): Debrief {
  ...
  /*
   * 🔴 РАЗБОР СДЕЛОК: ЧТО БРАЛ И МИМО ЧЕГО ПРОШЁЛ. Просьба Камиля: «добавь
   * разбор сделок, сейчас его в игре нет». Счётчики «купил 7 раз, пропустил
   * 12» человеку ничего не говорят — разбор ценят за имена: «прошёл мимо
   * пекарни за два миллиона, которая приносила бы 75 тысяч в месяц».
   *
   * Считаем по самой упущенной находке, а не по всем: список из двенадцати
   * строк никто не читает, а одна конкретная цифра запоминается.
   */
  const решения = l.решения ?? []
  const мимо = решения.filter((р) => !р.взял && р.поток > 0)
  const взял = решения.filter((р) => р.взял)
  if (мимо.length) {
    const худшая = [...мимо].sort((a, b) => b.поток - a.поток)[0]
    const упущено = мимо.reduce((n, р) => n + р.поток, 0)
    points.push({
      title: `Прошёл мимо ${мимо.length} находок`,
      text:
        `Самая заметная — «${худшая.что}»: взнос ${money(худшая.цена)}, приносила бы ` +
        `${money(худшая.поток)} в месяц. Всего мимо прошло ${money(упущено)} дохода в месяц — ` +
        `это ${Math.round((упущено / Math.max(1, expenses)) * 100)}% ваших расходов.` +
        (взял.length ? ` Взяли за партию ${взял.length}.` : ' Не взяли ни одной.'),
      tone: упущено > expenses ? 'bad' : 'neutral',
    })
    if (упущено > expenses)
      next.push(
        'Мимо прошло больше дохода, чем нужно для свободы. В жизни это выглядит так же: ' +
          'возможности приходят и уходят, а денег на них никогда не бывает «достаточно».',
      )
  }
