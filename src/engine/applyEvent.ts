  ...
  switch (e.type) {
    case 'PAYCHECK': {
      ...
      for (const b of l.businesses) {
        if (b.gl) {
          ...
          if (rank.level > next.rankPaid) {
             ...
             */
            /*
             * 🔴 Ранговые строки уходят В СВОЮ метку, а не в общий список
             * пояснений: из неё соберётся отдельная карточка, которая
             * покажется ПЕРЕД зарплатой. В общем списке они тонули.
             */
            const ранговые: string[] = []
            /*
            ...
            if (долиСтало > долиБыло) {
              ...
              ранговые.push(
                `В зачёт свободы пошло ${долиСтало}% дохода структуры вместо ` +
      ...
      for (const a of l.realEstate) {
        ...
        /*
         * 🔴 СРОК АРЕНДЫ ЗЕМЛИ ТАЕТ, И С НИМ ТАЕТ СТОИМОСТЬ. Доход идёт как
         * шёл — вилла работает; сгорает КАПИТАЛ, потому что покупатель платит
         * за остаток срока. Это и есть то, чего не видно в рекламной
         * доходности застройщиков: она считается так, будто актив вечный.
         */
        const ар = a.арендаЗемли
        if (ар && ар.осталосьМес > 0) {
          ар.осталосьМес -= 1
       ...
       */
      /*
       * 🔴 ТОЛЬКО В КРУГЕ. Решение Камиля по второму кругу: расходы там сами
       * не растут — уровень жизни человек уже выбрал, дальше он меняется
       * только карточками. Раньше проверка была не нужна: на Полосе шёл свой
       * день потока и сюда не заходил. Теперь второй круг крутит ту же
       * месячную машину, и без этого гейта уровень жизни начал бы догонять
       * доход ровно там, где игрок от него и убегает.
       */
      if (l.phase === 'ratRace') подтянутьРасходы(l, e.flowMul)
      ...
      /*
       * 🔴 ДОХОД ВДВОЕ ВЫШЕ РАСХОДОВ — РУБЕЖ, А НЕ ПОБЕДА (решение Камиля
       * 03.09, после замера). Сначала я сделал его условием победы, и замер
       * показал, чем это кончается: 90 побед из 90 приходили по доходу, а
       * мечтой — НИ ОДНОЙ. Двенадцать клеток мечты на большом поле стояли
       * впустую, потому что игра заканчивалась раньше, чем до них доходили.
       *
       * Теперь рубеж только отмечается: человек видит, что свободен, и идёт
       * за мечтой. Победа на втором круге одна — купить свою мечту.
       */
      if (l.phase === 'fastTrack' && !l.свободенС && свободаДостигнута(l, e.flowMul)) {
        l.свободенС = l.paydays
      }
      return l
    }
    ...
    case 'BUY_REAL_ESTATE':
      ...
      l.realEstate.push({
        ...
        profitSharePct: e.profitSharePct,
        /*
         * 🔴 Земля в аренде — срок начинает идти с покупки. Стоимость при
         * продаже с этого момента тает: покупатель платит за ОСТАТОК срока, а
         * не за вечность.
         */
        арендаЗемли: e.арендаЗемлиЛет
          ? { всегоЛет: e.арендаЗемлиЛет, осталосьМес: e.арендаЗемлиЛет * 12 }
          : undefined,
      })
      return l
    ...
    case 'BUY_BUSINESS':
      ...
      l.businesses.push({
        ...
        точкаОт: e.точкаОт,
        франшизаОт: e.франшизаОт,
        пассивное: e.пассивное,
     ...
     */
    /**
     * Выход из Круга: человек УВОЛИЛСЯ, а не продался.
     *
     * 🔴 РАНЬШЕ ЗДЕСЬ БЫЛ ВЫКУП, И ОН ЛОМАЛ ВТОРОЙ КРУГ. Активы стирались, а
     * вместо них в кошелёк падали пятьдесят месячных доходов — и та же сумма
     * становилась месячным доходом Полосы, то есть выдавалась дважды. Замер:
     * выкуп давал 20 875 000 ₽/мес при самой дешёвой мечте в 3 млн — цель
     * покупалась за один ход, деньги переставали быть ограничением, и от
     * второго круга оставалось «доехать кубиком». Отсюда и «зарплата 50
     * миллионов, за что?»: в жизни никто никого не выкупает.
     *
     * Теперь второй круг — продолжение первого. Квартиры, дела, бумаги и
     * партнёрский бизнес остаются вашими и продолжают работать по тем же
     * правилам. Уходит только зарплата и налоги с неё — вы уволились.
     *
     * 🔴 ДОЛГИ КРУГА ТОЖЕ ОСТАЮТСЯ. Прежде выкуп гасил всю ведомость разом;
     * теперь платить по ней приходится из дохода активов — и это честно: в
     * жизни увольнение долгов не отменяет. Гасить их на втором круге можно,
     * потому что месячная машина там та же (см. `advance` в table.ts).
     */
    case 'ENTER_FAST_TRACK': {
      if (l.phase !== 'ratRace') return prev
      /*
       * Налоги считались с зарплаты — вместе с ней и уходят. Остальные
       * расходы остаются: жизнь дешевле от увольнения не становится.
       */
      l.expenses.taxes = 0
      l.salary = 0
      l.phase = 'fastTrack'
      /*
       * 🔴 `beginningIncome` ОСТАВЛЕН НУЛЁМ, А НЕ УДАЛЁН. Поле читают старые
       * сохранённые партии: журнал переигрывается событие за событием, и если
       * поля не станет, чужая запись развалится на первом же дне потока.
       * Новым партиям он всегда ноль, и доход Полосы целиком идёт от активов.
       */
      l.fastTrack = {
        beginningIncome: 0,
        goalIncome: 0,
    ...
    /**
     * День потока на Полосе — ТОТ ЖЕ МЕСЯЦ, что и зарплата в Круге.
     *
     * 🔴 РАНЬШЕ ЭТО БЫЛИ ТРИ СТРОКИ, И ВТОРОЙ КРУГ ИЗ-ЗА НИХ ЗАМЕРЗАЛ. Вся
     * месячная механика — рост партнёрского бизнеса, его ранги и премии,
     * погашение рассрочек, срок просадок у дел — написана внутри ветки
     * зарплаты. На Полосе шёл этот отдельный день потока, который ничего из
     * перечисленного не трогал. Пока активы при выходе стирались, это было
     * незаметно. Как только они остаются, второй круг без месячной машины
     * превращается в застывшую картинку: партнёрский бизнес, дающий 94%
     * всего пассивного дохода, перестал бы расти в ту же секунду.
     *
     * Поэтому день потока теперь — это зарплата без зарплаты: та же ветка,
