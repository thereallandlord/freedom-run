 ...
 */
/*
 * Карточки-предложения: вложиться в своё дело.
 *
 * 🔴 ЧТО ИМЕННО ЛОВИМ. Движок применяет эффект вложения ОБЩИМ путём, передав
 * туда копию карточки без цены и без разовых денег (иначе за одно решение
 * списалось бы дважды). Значит карточка, у которой рядом с `вложить` стоит
 * `cash` или `dipPct`, потеряет их МОЛЧА: в тексте обещано, в партии не
 * случилось. Такую пару нельзя заметить за столом — только проверкой.
 */
const вложения = (marketCards('ru') as unknown as {
  id: string
  kind: string
  вложить?: number
  flowPct?: number
  ...
  categories?: string[]
  ремесло?: string[]
}[]).filter((c) => c.вложить != null)
console.log(`\nкарточек-предложений: ${вложения.length}`)
for (const c of вложения) {
  if (!c.вложить || c.вложить <= 0) беды.push(`${c.id}: цена вложения не положительная`)
  if (!c.flowPct) беды.push(`${c.id}: вложение без изменения дохода — решать нечего`)
  for (const поле of ['cash', 'dipPct', 'managerPct'] as const)
    if (c[поле] != null)
      беды.push(`${c.id}: рядом с «вложить» стоит ${поле} — движок его потеряет молча`)
}
/*
 * 🔴 ЦЕНА ДОЛЖНА ОКУПАТЬСЯ ЗА РАЗУМНЫЙ СРОК, иначе выбора нет: слишком дёшево —
 * берут не думая, слишком дорого — не берёт никто.
 *
 * 🔴 СЧИТАТЬ НАДО ПО ДОХОДУ НА РУКАХ (`instFlow`), а не по цифре с карточки.
 * Почти все покупают в рассрочку, и платёж съедает основное: автомойка обещает
 * 88 000, на руках остаётся 15 800. Первая версия этой проверки брала
 * несуществующее поле, молча получала NaN и не проверяла НИЧЕГО — цены были
 * выставлены на глаз под выдуманный поток и промахнулись вдвое.
 *
 * Мерим по СРЕДНЕМУ подходящему делу: крайности здесь врут в обе стороны —
 * капиталоёмкая мойка окупает что угодно за годы, а прачечная за полгода.
 */
for (const c of вложения) {
  const подходят = обычные.filter((b) => {
    const кат = (b as { category?: string }).category ?? ''
    const рем = (b as { ремесло?: string }).ремесло
    if (c.categories?.length && !c.categories.includes(кат)) return false
    if (c.ремесло?.length && (!рем || !c.ремесло.includes(рем))) return false
    return true
  })
  if (!подходят.length) {
    беды.push(`${c.id}: нет ни одного дела, к которому карточка подошла бы`)
    continue
  }
  const потоки = подходят
    .map((b) => Math.max(1, dealTerms(b as never, 'business').instFlow))
    .sort((a, b) => a - b)
  const средний = потоки[Math.floor(потоки.length / 2)]
  const вМесяц = (средний * Math.abs(c.flowPct ?? 0)) / 100
  const срок = (c.вложить ?? 0) / вМесяц
  if (срок < 5) беды.push(`${c.id}: окупается за ${срок.toFixed(1)} мес — берут не думая`)
  if (срок > 20) беды.push(`${c.id}: окупается за ${срок.toFixed(0)} мес — не возьмёт никто`)
}
if (вложения.length) {
  const прибавка = вложения.filter((c) => (c.flowPct ?? 0) > 0).length
  console.log(`  купить прибавку: ${прибавка} · удержать доход: ${вложения.length - прибавка}`)
}
