...
/**
 * Сколько ждём, прежде чем убрать связь, ушедшую в отказ. Запас нужен: в
 * отказ соединение попадает и при коротком провале сети, из которого оно
 * иногда выбирается само.
 */
const ПОРОГ_УБОРКИ_МС = 30_000
...
type Сигнал =
  | { t: 'привет'; from: string; имя: string; ответный?: boolean }
  | { t: 'оффер'; from: string; to: string; имя: string; sdp: string; поколение?: number }
  | { t: 'ответ'; from: string; to: string; sdp: string; поколение?: number }
...
export function создатьГолос(комната: string, я: { id: string; имя: string }): Голос {
  type Связь = {
    ...
    звонил: boolean
    /*
     * 🔴 НОМЕР СОГЛАСОВАНИЯ. «Мы держим какой-то свой оффер» — не то же самое,
     * что «этот ответ относится именно к нему». Связь теперь умеет
     * пересобираться, и между отправкой оффера и приходом ответа появилось
     * окно, где pc подменяется: опоздавший ответ от МЁРТВОГО соединения
     * проходил проверку состояния и садился на новый pc с чужими ключами, а
     * настоящий ответ отбрасывался молча. Номер сквозной для всего голоса
     * (не обнуляется при пересборке), поэтому старый ответ никогда не совпадёт.
     */
    поколение: number
  }
  let счётчикОфферов = 0
  ...
  let включён = false
  /*
   * 🔴 НОМЕР СЕАНСА. «Выключить» раньше начинался с `if (!включён) return`, а
   * `включён` выставляется ПОСЛЕ запроса микрофона — то есть всё время, пока
   * висит окно разрешения, гасить было нечего. Человек уходил из комнаты с
   * открытым окном, жал «Разрешить», и брошенный голос доигрывал запуск: живой
   * микрофон (индикатор записи горит до перезагрузки), вечный замер громкости
   * и связи с настоящей звуковой дорожкой — его слышали в комнате, которой у
   * него на экране уже не было.
   *
   * Тот же номер отсекает сигналы, доехавшие после выключения: они поднимали
   * связь БЕЗ микрофона (`мой` уже null), и она переживала повторное включение.
   */
  let сеанс = 0
  ...
  function создатьСвязь(id: string, имя: string): Связь {
    ...
    const связь: Связь = { pc, audio, имя, состояние: 'соединяемся', говорит: false, уровень: 0, лёдОчередь: [], звонил: false, поколение: 0 }
    ...
    pc.onconnectionstatechange = () => {
      ...
      изменилось()
      /*
       * 🔴 УШЕДШИЙ НАВСЕГДА ДОЛЖЕН УБИРАТЬСЯ САМ. Закрытая вкладка и севший
       * телефон не шлют «пока», и запись о человеке жила до конца партии:
       * незакрытое соединение, скрытый <audio> в теле страницы и — главное —
       * вечный таймер замера громкости, тикающий десять раз в секунду. За
       * партию со сменой людей таких призраков набирается столько, сколько
       * человек ушло. Полминуты в отказе — связь уже не оживёт, убираем.
       */
      if (s === 'failed' || s === 'closed') {
        setTimeout(() => {
          const текущая = связи.get(id)
          const st = pc.connectionState
          if (текущая === связь && (st === 'failed' || st === 'closed')) закрыть(id)
        }, ПОРОГ_УБОРКИ_МС)
      }
    }
    связи.set(id, связь)
  ...
  async function позвонить(id: string, имя: string) {
    ...
    связь.звонил = true
    связь.поколение = ++счётчикОфферов
    const offer = await связь.pc.createOffer({ offerToReceiveAudio: true })
    await связь.pc.setLocalDescription(offer)
    шлём({ t: 'оффер', from: я.id, to: id, имя: я.имя, sdp: offer.sdp ?? '', поколение: связь.поколение })
  }
  ...
  function приняли(s: Сигнал): Promise<void> {
    const сеансСигнала = сеанс
    очередьСигналов = очередьСигналов
      .then(() => {
        // Пока сигнал ждал своей очереди, голос могли выключить.
        if (сеансСигнала !== сеанс || !включён) return
        return обработать(s)
      })
      .catch((e) => {
  ...
  async function обработать(s: Сигнал) {
    if (s.t === 'привет') {
      ...
      /*
       * 🔴 ОТВЕЧАЕМ ТОЛЬКО НА ПЕРВОЕ «ПРИВЕТ», НЕ НА ОТВЕТНОЕ.
       *
       * Раньше отвечали на КАЖДОЕ — и двое здоровались бесконечно: моё
       * «привет» будило его ответ, его ответ будил мой. Канал один на всех,
       ...
       *
       * 🔴 РАЗЛИЧАЕМ ПО ПОМЕТКЕ, А НЕ ПО «ЗНАКОМ ЛИ ОН». Заманчиво молчать в
       * ответ всякому знакомому — и это ломает главный живой случай: человек
       * перезагрузил страницу. Он приходит с тем же ключом и пустой памятью, а
       * мы держим его в списке и молчим — он остаётся без звука НАВСЕГДА, и
       * ровно это делает каждый, кому сказали «обнови страницу».
       *
       * Поэтому первое «привет» от знакомого читаем как «я вернулся»: рвём
       * мёртвую связь и собираем заново. Обмен всё равно затухает после трёх
       * сообщений: его «привет» → мой ответный → его молчание.
       */
      if (!s.ответный) закрыть(s.from)
      const связь = связи.get(s.from) ?? создатьСвязь(s.from, s.имя)
      связь.имя = s.имя
      изменилось()
      if (!s.ответный) шлём({ t: 'привет', from: я.id, имя: я.имя, ответный: true })
    ...
    if (s.t === 'оффер') {
      ...
      // Номер не наш — просто возвращаем звонившему его же, чтобы он сверил.
      шлём({ t: 'ответ', from: я.id, to: s.from, sdp: ans.sdp ?? '', поколение: s.поколение })
      изменилось()
      return
    }
    if (s.t === 'ответ') {
      ...
      /*
       * Ждём ответ, только пока сами держим оффер, И ТОЛЬКО НА ТОТ САМЫЙ оффер.
       * Прежняя проверка «ещё нет удалённой стороны» после первого соединения
       * запирала связь навсегда; проверка одного лишь состояния пропускала
       * опоздавший ответ от уже мёртвого соединения.
       */
      if (!связь || связь.pc.signalingState !== 'have-local-offer') return
      /*
       * 🔴 БЕЗ НОМЕРА — НЕ ОТВЕРГАЕМ. В комнате всегда есть вкладки со старой
       * сборкой: они про номер не знают и присылают ответ без него. Строгая
       * сверка отрезала бы их наглухо — причём ровно тех, кто ПОСЛУШАЛСЯ и
       * обновил страницу: новый звонит старому и молча не слышит. Номер сверяем,
       * когда он есть; когда его нет, работаем как раньше.
       */
      if (s.поколение !== undefined && s.поколение !== связь.поколение) return
  ...
  return {
    async включить() {
      if (включён || запускается) return
      запускается = true
      const мойСеанс = ++сеанс
      ошибкаТекст = null
      ...
      /*
       * 🔴 ПОКА ВИСЕЛО ОКНО РАЗРЕШЕНИЯ, НАС МОГЛИ ПОГАСИТЬ. Тогда микрофон надо
       * отпустить своими руками: иначе он останется живым до перезагрузки —
       * индикатор записи горит, а голосового окна на экране уже нет.
       */
      if (сеанс !== мойСеанс) {
      ...
      запускается = false
      микВкл = true
      // Призраки от прошлого включения не должны пережить новое.
      ...
      канал.subscribe((st) => {
        ...
        /*
         * 🔴 ОБЪЯВЛЯЕМ «Я ЗДЕСЬ ЗАНОВО» — ЗНАЧИТ И У СЕБЯ РВЁМ ВСЁ СТАРОЕ.
         *
         * Этот обработчик срабатывает НЕ ТОЛЬКО на первой подписке: Supabase
         * переподключает канал после любого провала связи — переход с Wi-Fi на
         * мобильный, пробуждение ноутбука, свёрнутый телефон, — и шлёт то же
         * первое «привет». Собеседник по нему честно пересобирал связь с нуля,
         * а мы оставляли свою прежнюю вместе с отметкой «уже звонил» — и если
         * звонить полагалось нам, оффер не уходил больше НИКОГДА. Пара глохла
         * навсегда, без единой строки в консоли, от одного моргания сети.
         *
         * Уборка симметрична: обе стороны начинают с чистого листа. На первой
         * подписке связей ещё нет, так что это ничего не стоит.
         */
        for (const id of [...связи.keys()]) закрыть(id)
    ...
    выключить() {
      // Номер растим ВСЕГДА: этим гасится и запуск, застрявший на разрешении.
      сеанс++
      запускается = false
      if (!включён) return
