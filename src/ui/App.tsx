...
export function App() {
  ...
  /**
   * Хозяин стола временно ходит ЗА другого игрока.
   *
   * 🔴 Зачем. Партия на десятерых идёт вживую, и всегда кто-нибудь отходит,
   * у кого-то виснет телефон, у кого-то отвалилась сеть. Стол при этом стоит:
   * ход не перейдёт, пока человек не нажмёт. Раньше выход был один — сделать
   * его ботом насовсем. Теперь хозяин просто ходит за него, пока тот не
   * вернётся, и отдаёт место обратно одним нажатием.
   *
   * Механика бесплатная: движок и так берёт исполнителя из ПОДПИСИ хода, а не
   * из «чей ход». Значит достаточно подписываться местом того, за кого играем,
   * — и всё остальное (кнопки, левая панель, права) сходится само.
   */
  const [ходитЗа, setХодитЗа] = useState<string | null>(null)
  ...
  /*
   * Кем мы сейчас действуем. Обычно — своим местом; когда хозяин подхватил
   * чужой ход — местом того человека.
   *
   * 🔴 Подмена живёт, пока её не снимут: ходов может понадобиться несколько
   * (бросок, карточка, решение). Чтобы про неё нельзя было забыть, на экране
   * висит заметная полоса — она же и снимает.
   */
  const meSeatId = (isHost && ходитЗа) || meSeatIdBase
  ...
  const game = useGame(
    online
      ? {
          ...
          isHost,
          /*
           * 🔴 Подписываемся ТЕМ ЖЕ местом, что показывает экран. Раньше сюда
           * уходил `room.me.id`, а на экран — место, найденное по имени: у
           * того, кто перезашёл и получил новый ключ, экран показывал верное
           * место, а ходы подписывались старым — движок их отклонял, и у
           * одного человека за столом «не работали кнопки».
           */
          meId: meSeatId,
          // Настоящее моё место: по нему стол поймёт, что ход сделан за другого.
  ...
  return (
    <Landing
      ...
      /*
       * 🔴 Садимся играть за одним экраном — ЗАБЫВАЕМ прошлую комнату.
       *
       * Комната поднималась из браузера при каждом запуске и жила, пока её
       * не покинешь явно: уйти на главную или закрыть вкладку было
       * недостаточно. А пока комната «жива», игра считает партию сетевой и
       * подписывает каждое действие местом из комнаты — за местным столом
       * такого места нет, и движок отклонял всё подряд. Человек жал «Бросок»,
       * и не происходило ничего.
       */
      /*
       * 🔴 КОМНАТУ БОЛЬШЕ НЕ ЗАБЫВАЕМ. Забывали ради того, чтобы подпись из
       * комнаты не ломала местную партию, — но это давно решено иначе:
       * useGame определяет местную партию по САМОМУ СЕТАПУ (места с экрана
       * настройки безымянные) и сетевой объект ей не выдаёт вовсе.
       *
       * А вред был настоящий: человек вышел из онлайн-партии на главную,
       * заглянул в «На одном устройстве» — и комната стёрлась, а сохранение
       * сетевой партии осталось. Дальше «Продолжить» сажало его за тот же
       * стол, но уже НЕ сетевой: ходы никуда не уходили, голос и подхват хода
       * исчезали. Ровно на это Камиль и наткнулся.
       */
