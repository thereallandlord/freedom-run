    ...
    <div>
      ...
      {open && (
        <div className="mb-1 ml-3 space-y-0.5 border-l border-[var(--t-line, var(--line))] pl-2 text-[11px] text-[var(--t-muted, var(--muted))]">
            ...
            <>
              ...
              */}
              {(() => {
                 ...
                 * правил рассрочки (жильё 180 месяцев, дело 84), а не числом
                 * в комментарии: срок уже менялся, и вшитая цифра соврала бы.
                 */
          ...
          */}
          {/*
            🔴 СРОК АРЕНДЫ ЗЕМЛИ ВИДЕН ТАМ, ГДЕ ЧЕЛОВЕК СМОТРИТ НА СВОЙ АКТИВ.
            Доход у такого объекта идёт как у обычного, а капитал тает — и без
            этой строки таяние выглядело бы поломкой: «почему за виллу дают
            меньше, чем я отдал?».
          */}
          {(a as RealEstateAsset).арендаЗемли ? (
            <div className="mt-1 rounded-lg bg-amber-500/10 px-2 py-1.5 text-[10.5px] leading-snug text-amber-700 dark:text-amber-300">
              Земля в аренде: осталось{' '}
              {Math.round((a as RealEstateAsset).арендаЗемли!.осталосьМес / 12)} лет из{' '}
              {(a as RealEstateAsset).арендаЗемли!.всегоЛет}. Приносит как обычно, но при продаже
              с каждым годом дают меньше — платят за остаток срока.
            </div>
          ) : null}
  ...
  return (
    ...
    */
    <div className="space-y-2 pb-4 pt-3">
      ...
      {/*
        🔴 НА ВТОРОМ КРУГЕ БЛОК ДОБАВЛЯЕТСЯ, А НЕ ЗАМЕНЯЕТ ВСЁ ОСТАЛЬНОЕ.
        Раньше панель на Полосе показывала ТОЛЬКО цель и прятала доходы,
        расходы и активы — потому что активов там и не было, их выкупали.
        Теперь они остаются с человеком, и спрятанная панель означала бы, что
        своим кафе на втором круге распорядиться нельзя: ни продать, ни
        вложиться, ни нанять управляющего. Показываем и то, и другое.
      */}
      {onFast && (
        /*
