        ...
        >
          ...
          */}
          <div className="panel-2 space-y-1 rounded-lg p-3">
            ...
            {/*
              🔴 ОБЪЕКТ, КОТОРЫЙ СЪЕДАЕТ ДЕНЬГИ, ОБЯЗАН СКАЗАТЬ ЭТО СЛОВАМИ.
              После перехода на настоящие цены и аренду таких стало пятнадцать:
              платёж по рассрочке больше аренды, и каждый месяц владения стоит
              денег. Это не поломка, а правда — так и живёт съёмная квартира в
              ипотеку. Но человек должен решать, зная: минус в строке легко
              принять за мелочь, если рядом крупная цифра дохода.
            */}
            {terms.financeable &&
            marketDealFlow(terms.cashFlow, mktMul) - terms.instMonthly < 0 ? (
              <p className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-2.5 py-2 text-[11.5px] leading-snug text-amber-700 dark:text-amber-300">
                В рассрочку этот объект будет СЪЕДАТЬ{' '}
                {money(terms.instMonthly - marketDealFlow(terms.cashFlow, mktMul))} каждый месяц:
                платёж больше аренды. Он всё равно растёт в цене и его можно продать — но дохода
                не даёт, пока не выкуплен.
              </p>
            ) : null}
            {growth ? (
              <Stat
          ...
          */}
          ...
          {/*
            🔴 ЗЕМЛЯ В АРЕНДЕ — САМОЕ ВАЖНОЕ ПРО ЭТОТ ОБЪЕКТ, И МОЛЧАТЬ О НЁМ
            НЕЛЬЗЯ. Разбор живых объявлений поймал то, чего мы не знали: на
            Бали иностранцу землю в собственность не продают, все сделки — на
            19–30 лет. Цена на карточке не покупка, а плата за срок. Пишем это
            рядом с ценой, а не мелким шрифтом внизу: человек решает, зная.
          */}
          {(card as { арендаЗемлиЛет?: number }).арендаЗемлиЛет ? (
            <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2.5 text-[12px] leading-snug">
              <div className="font-bold text-amber-600 dark:text-amber-400">Земля в аренде</div>
              <div className="mt-0.5 text-[var(--muted)]">
                Иностранцу здесь землю не продают: договор на{' '}
                {(card as { арендаЗемлиЛет?: number }).арендаЗемлиЛет} лет, потом дом вернётся
                владельцу земли. Приносить будет как обычная недвижимость, а вот стоить при
                продаже — всё меньше с каждым годом.
              </div>
            </div>
          ) : null}
