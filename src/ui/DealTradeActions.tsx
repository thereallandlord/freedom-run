...
export function DealTradeActions({
  ...
  /*
   * 🔴 «ВСЕМ ЗА СТОЛОМ» ПОНИМАЛИ КАК СКЛАДЧИНУ НА ВСЕХ. Живая путаница:
   * «сумма входа партнёру — на всех или с каждого?». Предложение уходит всем,
   * но принимает его ОДИН — кто успел; сумма от этого не делится. Пишем это
   * прямо в подписи, а не в правилах, которых никто не читает.
   */
  const who: Option<string>[] = [
    { value: 'all', label: 'Всем — кто первый согласится', hint: 'сумма не делится' },
