...
export function Game({
  ...
  const canEscape =
    ...
    !rolled
  /*
   * 🔴 КНОПКА НАЗЫВАЕТ ТО, С ЧЕМ ЧЕЛОВЕК ОСТАНЕТСЯ. Раньше она обещала выкуп —
   * сумму, за которую его «покупали», — и его же приходилось вычищать от
   * долгов и долей, чтобы не соврать. Выкупа больше нет: увольнение ничего не
      ...
      */}
      ...
      {table.pending && table.pending.kind !== 'gameOver' && (
        <CardModal
           ...
           */
          /*
           * 🔴 ВЛАДЕЛЕЦ ХОДА НИКОГДА НЕ ЗРИТЕЛЬ НА СВОЕЙ КАРТЕ. Список
           * ожидающих может опустеть на ровном месте (сосед продал бумаги и
           * перестал быть заинтересованным) — и тогда «вас не ждут» гасило
           * кнопки У ВСЕХ, включая того, чей ход. Выйти было нечем: движок
           * разрешает владельцу снять карту вторым нажатием, а нажать
           * стало негде. Ход мой — кнопки есть всегда.
           */
          spectate={
            table.pending?.kind === 'deal' || table.pending?.kind === 'market'
              ? !myTurn && !pendingUndecided(table).some((x: Seat) => x.id === seat.id)
              : !myTurn && !allowedIn
          }
