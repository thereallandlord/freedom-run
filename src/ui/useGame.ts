...
export function useGame(net?: {
   ...
   */
  /*
   * 🔴 СЕТЕВАЯ ЛИ ЭТА ПАРТИЯ — РЕШАЕТ СТОЛ, А НЕ НАЛИЧИЕ КОМНАТЫ.
   *
   ...
   * когда-то зайти в комнату, а сейчас играть за одним экраном. Раньше в этом
   * случае игра считала местную партию сетевой — и ложилась целиком: каждое
   * действие подписывалось местом из комнаты, а движок такую подпись
   * отвергает, потому что места с таким именем за столом нет.
   *
   * Признак простой и самопроверяющийся: моё место из комнаты ЕСТЬ за этим
   * столом. Пока стол не собран (лобби, первый кадр) — верим комнате, иначе
   * настоящая сетевая партия не смогла бы начаться.
   *
   * 🔴 Одной подписи мало: у хода есть ещё и отправка в канал. Если партия
   * местная, а мы всё равно шлём ходы в комнату, бот на «Продолжить» встаёт
   * намертво — его конец хода уходит в канал вместо стола. Поэтому признак
   * один на ОБА решения: и подпись, и отправку.
   */
  const meId = net?.meId
  const сетевая = !!net && (!table || !meId || table.seats.some((s) => s.id === meId))
  const netSend = сетевая ? net?.send : undefined
  const dispatch = useCallback(
    (e: TableEvent) => {
       ...
       */
      const signed = сетевая && meId ? ({ ...e, by: meId } as TableEvent) : e
      if (netSend) {
        /*
    ...
    [applyLocal, meId, netSend, сетевая],
  )
