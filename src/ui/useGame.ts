...
function отпечатокПравил(): string {
   ...
   * загрузку страницы.
   */
  const полные = полныеКолоды('ru')
  const всё = JSON.stringify([
    /*
     * 🔴 ВЕРСИЯ РАЗДАЧИ КАРТ — РУКАМИ. Отпечаток считается по СОДЕРЖИМОМУ
     * колод, и этого мало: правка в том, КАК движок тянет карту, содержимое
     * не меняет, а журнал после неё переигрывается в другую партию — молча.
     * Так и вышло 28.08, когда обычный рынок отделили от личных колод.
     * Меняешь порядок или условия выдачи карт — увеличь число.
     */
    'раздача:2',
    /*
     * 🔴 ПРАВИЛА ВТОРОГО КРУГА — тоже часть партии. Выход из Круга перестал
     * быть выкупом, а победа — доходом; журнал, записанный при старых
     * правилах, переигрывается в совсем другую игру, и отпечаток обязан это
     * заметить. Меняешь устройство второго круга — увеличь число.
     */
    'второйкруг:1',
    /*
     * 🔴 КОЛОДЫ ЦЕЛИКОМ, МИМО ВЫБОРА СТРАН. Отпечаток обязан считаться
     * одинаково при любом составе стола: он проверяет, теми ли ДАННЫМИ
     * записан журнал, а какими странами играли — записано в настройках и
     * приезжает вместе с журналом. Считался бы он по отфильтрованной колоде
     * — партия «без России» не пережила бы перезагрузку страницы.
     */
    полные.small,
    полные.big,
    полные.market,
    ...
    RAT_BOARD,
    /*
     * 🔴 БОЛЬШОЕ ПОЛЕ ЗАБЫЛИ. Отпечаток брал только доску Круга, а на Полосе
     * цены мечты и дела — такие же данные партии. Поправка цен мечты под
    ...
    полнаяПолоса(),
    /*
     * 🔴 ПРОФЕССИИ ЗАБЫЛИ ТАК ЖЕ, КАК БОЛЬШОЕ ПОЛЕ. Зарплата и расходы
     * профессии попадают в партию при создании стола, и журнал, записанный
     * со старыми расходами, переигрался бы с новыми — другие деньги, другой
     * выход из Круга, без единого слова. Всплыло 10.09, когда расходы всех
...
export function useGame(net?: {
   ...
   */
  /*
   * 🔴 ЗАМОК НА ВРЕМЯ БРОСКА — ОДИН НА ВСЕ КНОПКИ СРАЗУ.
   *
   * Кубик летит 1,55 с, и всё это время стол ещё стоит в фазе «ждём броска»:
   * движок про начатый бросок не знает. Любая кнопка, оставшаяся в эту
   * секунду на экране, запускала ВТОРОЙ бросок. Он доезжал до движка и молча
   * отклонялся (фаза уже другая), а человек досматривал второй крутящийся
   * кубик и не понимал, что произошло: ни строки в журнале, ни плашки.
   *
   * Хуже того, отклонённый бросок всё равно уходил в общий журнал комнаты и
   * становился якорем для «Отменить»: отмена снимала пустышку, а ход
   * оставался на месте. Замерено — это и есть старая жалоба «отмена ничего
   * не даёт».
   *
   * Замок держим в ref, а не в состоянии: состояние приезжает следующим
   * рендером, а два нажатия успевают пройти раньше. Прятать кнопку по
   * `rolling`/`rolled` мало — это защита экрана, а нужна защита действия.
   */
  const бросокИдёт = useRef(false)
  const roll = useCallback(
    (count: number) => {
      if (бросокИдёт.current) return
      бросокИдёт.current = true
