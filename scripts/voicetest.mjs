...
console.log('\n6. «Выключить» во время запроса микрофона отпускает микрофон')
{
  ...
  // Возвращаем обычный микрофон — иначе следующая проверка ждёт разрешения вечно.
  поставитьПодпорки()
}

console.log('\n7. Старая вкладка в одной комнате с новой — обе слышат друг друга')
{
  ФейкPC.все.length = 0
  const шина = создатьШину()
  const поднять = async (id, имя) => {
    globalThis.__КЛИЕНТ__ = создатьКлиент(шина, id)
    const г = создатьГолос('к7', { id, имя })
    await г.включить()
    return г
  }
  await поднять('aaa', 'Анна') // новый код, меньший ключ ⇒ звонит он
  await поднять('zzz', 'Борис')
  /*
   * Борис — «старая вкладка»: его сборка про номер согласования не знает и
   * отправляет ответ без него. Режем поле прямо на шине, у его канала.
   */
  const бк = каналКого(шина, 'zzz')
  const было = бк.send.bind(бк)
  бк.send = (m) => {
    const p = m?.payload
    if (p && (p.t === 'ответ' || p.t === 'оффер')) delete p.поколение
    return было(m)
  }
  await осесть()
  const сОтветом = ФейкPC.все.filter((pc) => pc.remoteDescription?.type === 'answer')
  проверить(сОтветом.length >= 1, `ответ старой вкладки принят (соединений с ответом: ${сОтветом.length})`)
}
