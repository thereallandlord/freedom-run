...
// src/engine/diag.ts
function trace(theme, seed) {
  setActiveTheme(theme);
  setFastBoardTheme(theme);
  setRules(theme === "ru" ? { currency: "RUB", fastTrackMultiplier: 50, fastTrackTarget: 1e6, loansEnabled: false } : { currency: "USD", fastTrackMultiplier: 100, fastTrackTarget: 15e4, loansEnabled: true });
  const dreams = dreamSpaces();
  const pool = professionsFor(theme);
  const r0 = mulberry32(seed);
  const setup = { seed, deckTheme: theme, seats: [0, 1].map((i) => ({
    name: `B${i}`,
    professionId: pool[Math.floor(r0() * pool.length)].id,
    dreamSpace: dreams[Math.floor(r0() * dreams.length)].index,
    isBot: true,
    botDifficulty: i ? "high" : "medium"
  })) };
  let t = createTable(setup);
  const p0 = t.seats[0].ledger;
  console.log(`
  ${p0.profession.name}: \u0437\u0430\u0440\u043F\u043B\u0430\u0442\u0430 ${p0.salary.toLocaleString("ru")} \xB7 \u0440\u0430\u0441\u0445\u043E\u0434\u044B ${totalExpenses(p0).toLocaleString("ru")} \xB7 \u043F\u043E\u0442\u043E\u043A +${(p0.salary - totalExpenses(p0)).toLocaleString("ru")} \xB7 \u0441\u0442\u0430\u0440\u0442 ${p0.cash.toLocaleString("ru")}`);
  const rnd = mulberry32(seed ^ 1597334677);
  let ev = 0, stuck = 0;
  const buys = [];
  while (t.phase !== "finished" && !t.seats.some((s) => s.track === "fast") && ev < 2e4) {
    const before = t;
  ...
  console.log(`  \u0432\u044B\u0448\u0435\u043B \u0437\u0430 ${t.turnCounter} \u0445\u043E\u0434\u043E\u0432 \xB7 \u043F\u0430\u0441\u0441\u0438\u0432 ${passiveIncome(l).toLocaleString("ru")} \u043F\u0440\u043E\u0442\u0438\u0432 \u0440\u0430\u0441\u0445\u043E\u0434\u043E\u0432 ${totalExpenses(l).toLocaleString("ru")}`);
  buys.slice(0, 6).forEach((b) => console.log(`    \xB7 ${b}`));
}
console.log("=== \u041A\u041B\u0410\u0421\u0421\u0418\u041A\u0410 ===");
[1e3, 1074].forEach((s) => trace("classic", s));
console.log("\n=== \u0420\u041E\u0421\u0421\u0418\u042F ===");
[1e3, 1074].forEach((s) => trace("ru", s));
