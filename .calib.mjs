...
function run(seed, diff, theme) {
  ...
  while (t.phase !== "finished" && !t.winnerId && ev < 2e4) {
    const before = t;
...
function measure(theme, label) {
  const rs = Array.from({ length: 30 }, (_, i) => run(1e3 + i * 37, mixes[i % 4], theme));
  const avg = (f) => Math.round(rs.reduce((s, r) => s + f(r), 0) / rs.length);
  console.log(`  ${label.padEnd(34)} \u0432\u0441\u0435\u0433\u043E ${String(avg((r) => r.turns)).padStart(4)} \xB7 \u041A\u0440\u0443\u0433 ${String(avg((r) => r.escapeTurn)).padStart(4)} \xB7 \u041F\u043E\u043B\u043E\u0441\u0430 ${String(avg((r) => r.ftTurns)).padStart(4)}`);
  return avg((r) => r.turns);
}
console.log("\n=== \u042D\u0442\u0430\u043B\u043E\u043D ===");
setRules({ currency: "USD", fastTrackMultiplier: 100, fastTrackTarget: 15e4, loansEnabled: true });
var target = measure("classic", "classic (\u044D\u0442\u0430\u043B\u043E\u043D)");
...
var flows = board.filter((s) => s.type === "business").map((s) => s.cashFlow);
console.log(`  \u043F\u043E\u0442\u043E\u043A \u0438\u043D\u0432\u0435\u0441\u0442\u0438\u0446\u0438\u0439 \u041F\u043E\u043B\u043E\u0441\u044B: ${Math.min(...flows)}\u2013${Math.max(...flows)} \u20BD/\u043C\u0435\u0441, \u043C\u0435\u0434\u0438\u0430\u043D\u0430 ${flows.sort((a, b) => a - b)[Math.floor(flows.length / 2)]}`);
for (const goal of [1e6, 2e6, 3e6, 4e6, 5e6]) {
  setRules({ currency: "RUB", fastTrackMultiplier: 50, fastTrackTarget: goal, loansEnabled: false });
  measure("ru", `\u0446\u0435\u043B\u044C ${(goal / 1e6).toFixed(0)} \u043C\u043B\u043D \u20BD/\u043C\u0435\u0441`);
}
console.log(`
